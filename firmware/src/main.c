...
/*
    ...
    // primo sample
    //SAMPLE ADDRE --> da dove inizio a leggere i sample
    spi_load_sample(SAMPLE_ADDR,  CHANNELS*16);
    sample_addr   = SAMPLE_ADDR + CHANNELS*2;
...
static void send_inference(void)
{
    uint32_t s;
    do {
        // durante il polling dell’inferenza, prova a caricare le istruzioni
        //check_and_load_next_instr();
