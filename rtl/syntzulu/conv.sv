...
always @(posedge clk)
    if (rst)
        ...
        en_conv_shift[0] <= en_conv;
        en_conv_shift[1] <= en_conv_shift[0];
        en_conv_shift[2] <= en_conv_shift[1];
        en_conv_shift[3] <= en_conv_shift[2];
        en_conv_shift[4] <= en_conv_shift[3];
    end
...
// output assignment
//assign valid_all = en_shift[PIPE-1];
