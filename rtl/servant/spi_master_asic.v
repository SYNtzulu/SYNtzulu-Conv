...
module spi_master_asic(
	...
	begin
		if(reset == 1) begin
			state <= IDLE;
			// ASIC: same values IDLE loads, but from the reset branch. With
			// only "state" reset, these flops stay at their random power-up
			// value for one cycle after the reset release: end_transaction
			// feeds the mm_spi_valid handshake in servant_spi, which latches
			// that garbage on the very first clock edge and the CPU reads it
			// during the boot polling (X in gate level simulation).
			SPI_SCK <= 0;
			valid <= 0;
			counter_clk <= 0;
			counter_send <= 0;
			...
			read_req_r <= 0;
			words_to_read_reg <= 0;
			words_to_read_reg_sub0 <= 0;
			words_to_read_reg_sub1 <= 0;
		end else begin
			case (state)
